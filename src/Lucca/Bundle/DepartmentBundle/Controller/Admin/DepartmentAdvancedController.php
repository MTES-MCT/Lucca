...
class DepartmentAdvancedController extends AbstractController
{
    ...
    #[IsGranted("ROLE_SUPER_ADMIN")]
    public function generateSettingsAction(Department $department): Response
    {
        try {
            $this->settingGenerator->generateMissingSettings($department);
            $this->addFlash('success', 'flash.settings.createdSuccessfully');
        }catch (\Exception $e){
            $this->addFlash('danger', 'flash.settings.createdError');
        }
